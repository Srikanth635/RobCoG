...
ASLLoggerManager::ASLLoggerManager()
{
	...
	UE_LOG(LogTemp, Log, TEXT("Inside SLLoggerManager Constructor -----------------------------------"))
	// Default values
	bIsInit = false;
	bIsStarted = false;
	bIsFinished = false;
...
{
	Super::BeginPlay();
	UE_LOG(LogTemp, Log, TEXT("Inside SLLoggerManager BeginPLay -----------------------------------"))
	if (bUseIndependently)
	{
...
void ASLLoggerManager::Init()
{
	...
	bIsInit = true;
	UE_LOG(LogTemp, Log, TEXT("Inside Fucking Constructor -----------------------------------"))
...
void ASLLoggerManager::Finish(bool bForced)
{
	...
	if (bLogActionsAndEvents)
	{
		...
		SemanticData = SymbolicLogger->semanticData;
	}
