...
int32 FSLWorldStateDBWriterAsyncTask::AddAllIndividuals(bson_t* doc)
{
	...
	for (const auto& Individual : IndividualManager->GetIndividuals())
	{
		...
		BSON_APPEND_DOCUMENT_BEGIN(&arr_obj, idx_key, &individual_obj);
			...
			FString IDx = Individual->GetIdValue();
			FString Eye = TEXT("tuS9i8Qd1kOwG71fUlWehw");
			if (IDx == Eye)
			{
				AActor* parentActor = Individual->GetParentActor();
				FString HitActor = parentActor->GetActorLabel();
				//UE_LOG(LogTemp,Warning,TEXT("Name of the parent (WriteALL) %s"),*FString(HitActor));
				BSON_APPEND_UTF8(&individual_obj,"Hit Actor",TCHAR_TO_UTF8(*HitActor));
			}
			// Pose
