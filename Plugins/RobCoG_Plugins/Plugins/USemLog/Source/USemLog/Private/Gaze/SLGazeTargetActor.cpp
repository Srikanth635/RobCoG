...
#include "C:/Program Files/Epic Games/UE_4.27/Engine/Source/Runtime/Core/Public/Logging/LogMacros.h"
...
#if SL_WITH_EYE_TRACKING
	UE_LOG(LogTemp, Warning, TEXT("Inside SL_WITH_EYE_TRACKING if condition"))
...
void ASLGazeTargetActor::Update()
{
	...
	if (GazeProxy->GetRelativeGazeDirection(RelativeGazeDirection))
	{
		...
		else
		{
			...
			{
				SetActorLocation(HitResult.ImpactPoint);
				FVector MyImpact = FVector(HitResult.ImpactPoint);
				AActor* ActorHit = HitResult.GetActor();
				FString ActorName = ActorHit->GetName();
				float time = float(HitResult.Time);
				float realtimeSeconds = UGameplayStatics::GetRealTimeSeconds(GetWorld());
				FString timeStamp = FString::SanitizeFloat(realtimeSeconds);
				hitActor = "," + ActorName + "\n";
				if (IsActorLabelEditable())
				{
					ClearActorLabel();
					//UE_LOG(LogTemp, Warning, TEXT("Label is editable--------------------"));
					SetActorLabel(ActorName,false);
				}
				SetActorRotation(HitResult.ImpactNormal.ToOrientationQuat());
			}
		}
