...
namespace UnrealBuildTool.Rules
{
	public class SpeechRecognition : ModuleRules
	{
		...
		public SpeechRecognition(ReadOnlyTargetRules Target) : base(Target)
        {
            ...
            LoadWhisper(Target);
            //Log.TraceInformation(sf.ToString());
            //System.Console.WriteLine(sf.ToString());
            LoadSphinxBase(Target);
            LoadPocketSphinx(Target);
		}

        public bool LoadWhisper(ReadOnlyTargetRules Target)
        {
            ...
            if ((Target.Platform == UnrealTargetPlatform.Win64) || (Target.Platform == UnrealTargetPlatform.Win32))
            {
                ...
                string LibrariesPath = Path.Combine(ThirdPartyPath, "Whisper", "Libraries");
                LibrariesPath = Path.Combine(LibrariesPath, PlatformString);

                PublicAdditionalLibraries.Add(Path.Combine(LibrariesPath, "whisper.lib"));
                PublicAdditionalLibraries.Add(Path.Combine(LibrariesPath, "libopenblas.lib"));
                PublicAdditionalLibraries.Add(Path.Combine(LibrariesPath, "SDL2main.lib"));
                PublicAdditionalLibraries.Add(Path.Combine(LibrariesPath, "SDL2test.lib"));

                // TODO: Copy dlls to alternative package directory, to be loaded through a manual process
                PublicDelayLoadDLLs.Add("whisper.dll");
                RuntimeDependencies.Add(new RuntimeDependency(Path.Combine(Path.Combine(LibrariesPath, "whisper.dll"))));

                PublicDelayLoadDLLs.Add("SDL2.dll");
                RuntimeDependencies.Add(new RuntimeDependency(Path.Combine(Path.Combine(LibrariesPath, "SDL2.dll"))));

                PublicDelayLoadDLLs.Add("libopenblas.dll");
                RuntimeDependencies.Add(new RuntimeDependency(Path.Combine(Path.Combine(LibrariesPath, "libopenblas.dll"))));         
            }
            ...
            {
                // Include path
                PublicIncludePaths.Add(Path.Combine(ThirdPartyPath, "Whisper", "Includes"));
                PrivateIncludePaths.Add(Path.Combine(ThirdPartyPath, "Whisper", "Includes"));
            }

            Definitions.Add(string.Format("WITH_WHISPER_BINDING={0}", isLibrarySupported ? 1 : 0));
